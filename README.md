...
![A PORT 70 cabin bag shown in context](assets/context/ryan-airplane-seat-black.png)
...
```text
Shopper
  expresses a need and remains in control of the purchase
        ↓
AI companion
  shares only relevant, user-known context and invokes page tools
        ↓
WebMCP
  provides structured local actions in the active browser page
        ↓
PRSIM resolver
  matches the request to the closest merchant-authored scenario
        ↓
Storefront
  renders approved layouts, copy, product truth, assets and evidence
```
...
> I found this store. Help me choose a bag for a three-day Ryanair trip to Dublin. My budget is €100, I want black, and under-seat compatibility is my priority.

**Companion action**

```js
prepare_shopping_experience({
  request: "Three-day Ryanair trip to Dublin; maximum €100; black; under-seat fit is the priority.",
  use_language: "en",
  contexts: ["air_low_cost"],
  context_details: {
    airline: "ryanair",
    fare: "personal_item",
    destination: "Dublin",
  ...
  constraints: {
    budget_max_eur: 100,
    underseat_required: true
  },
  practical_needs: ["price", "compatibility", "weather_protection"],
  aesthetic_preferences: {
    color: "black",
    product_expression: "discreet"
  }
})
```
...
The same page becomes a focused low-cost flight experience: an under-seat recommendation, airline dimensions, contextual imagery, relevant rain information and an explicit purchase action. A later request such as “Show me Liberty blue” invokes only `choose_color`.
...
demo-companion/           demo instructions, personas and presentation
```
